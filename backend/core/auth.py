...
def _clerk_frontend_api_base() -> str | None:
    return settings.CLERK_FRONTEND_API_URL or settings.CLERK_FRONTEND_API


def _clerk_jwt_audience() -> str | None:
    return settings.CLERK_JWT_AUDIENCE or settings.CLERK_AUDIENCE
...
def _jwks_url_candidates(unverified_claims: dict[str, Any]) -> list[str]:
    ...
    frontend_api_base = _clerk_frontend_api_base()
    if frontend_api_base:
        candidates.append(f"{frontend_api_base.rstrip('/')}/.well-known/jwks.json")
...
def verify_clerk_token(token: str) -> AuthenticatedUser:
    ...
    jwt_audience = _clerk_jwt_audience()
