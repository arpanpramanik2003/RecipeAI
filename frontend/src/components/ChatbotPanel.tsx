...
const ChatbotPanel = ({ currentRecipe }: ChatbotPanelProps) => {
  ...
  return (
    <div className="flex flex-col h-[650px] glass rounded-3xl overflow-hidden shadow-xl border border-border">
      {/* Header */}
      <div className="h-16 flex items-center px-6 border-b border-border bg-muted/40 backdrop-blur-md">
        <Bot className="w-6 h-6 text-primary mr-3" />
        <h3 className="font-bold text-foreground text-lg">AI Sous-Chef</h3>
      </div>

      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6 flex flex-col scrollbar-hide">
        {messages.map((msg, i) => (
          <motion.div 
          ...
          >
            <div className={`max-w-[85%] rounded-2xl p-4 sm:p-5 shadow-sm border ${
              msg.role === 'user' 
        ...
        {isLoading && (
          ...
          >
            <div className="bg-muted/40 text-foreground rounded-2xl rounded-bl-sm p-5 w24 flex items-center justify-center border border-border/80 backdrop-blur-xl shadow-sm">
      ...
      {/* Input Area */}
      <div className="p-5 border-t border-border bg-background/50 backdrop-blur-lg">
        <div className="relative flex items-center shadow-md rounded-2xl bg-muted/30 border-2 border-border/80 focus-within:ring-4 focus-within:ring-primary/20 focus-within:border-primary/50 transition-all duration-300 group focus-within:shadow-[0_0_30px_hsl(330_80%_60%_/_0.15)]">
          <input
            ...
            className="w-full h-16 pl-6 pr-16 bg-transparent border-none outline-none text-[15px] font-medium placeholder-muted-foreground/60"
            disabled={isLoading}
          />
          <button
            onClick={handleSend}
            disabled={!input.trim() || isLoading}
            className="absolute right-2 w-12 h-12 rounded-xl flex items-center justify-center text-white bg-gradient-to-br from-purple-500 to-orange-500 disabled:opacity-40 transition-all duration-300 active:scale-95 shadow-md shadow-primary/20"
          >
